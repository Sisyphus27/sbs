# Graph Report - sbs  (2026-07-23)

## Corpus Check
- 235 files · ~2,477,918 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2019 nodes · 3879 edges · 161 communities (149 shown, 12 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 74 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2a30e590`
...
Cohesion: 0.06
Nodes (46): Flask, is_legal_combo(), cycle_number(), Which 21-week cycle a program week falls in (1-based)., app(), test_get_and_replace_schedule(), test_reset_schedule_restores_defaults(), test_cycle_number() (+38 more)
...
Cohesion: 0.17
Nodes (21): Week-2 plan view pulls intensity/reps/repout from sbs_schedule, not lifts column, test_plan_view_shows_week2_schedule_values(), _sbs(), _t2(), test_actual_tonnage_basic(), test_actual_tonnage_single_set(), test_actual_tonnage_zero_or_none_sets_falls_back_to_3(), test_t2_target_as_of_initial_when_no_prior_history() (+13 more)
...
Cohesion: 0.16
Nodes (31): Profile, advance_lift(), initial_state(), Apply this week's logged last-set reps; mutate state in place.      All per-mo, Re-derive a linear_t2/linear_t3 lift's state by replaying progression     from, recompute_state(), _bw_profile(), _profile() (+23 more)
...
Cohesion: 0.11
Nodes (41): Lift, LiftState, A lift definition in profile.yaml (static)., Per-lift dynamic state in state.yaml., get_mode(), test_guard_advance_progression_none_keeps_added_zero(), test_guard_advance_t2_reset_uses_working_weight(), Tests for the progression-mode registry (ADR 0005). (+33 more)

### Community 9 - "Advance service & DB tests"
Cohesion: 0.23
Nodes (12): _profile_from_rows must carry bodyweight from settings into Profile so     the, Same exercise on two days is two independent rows; logging by id targets each., _seed(), test_advance_week_handles_duplicate_names_per_day(), test_advance_week_rows_t2_hit_increments(), test_advance_week_runs_engine_and_bumps_week(), test_advance_week_skips_unlogged_lifts(), test_profile_from_rows_maps_bodyweight() (+4 more)

### Community 10 - "Settings routes & autosave"
Cohesion: 0.07
Nodes (13): LinearT2Mode, LinearT3Mode, Mode, Pure-bodyweight record-only mode: no automatic progression (ADR 0005)., Base progression-mode handler. Subclasses override the four ops., Return {weight, reps, repout, target, streak} for week_plan display., Return the new-mode starting state dict (tm/weight/target/streak)., RecordOnlyMode (+5 more)

### Community 11 - "Lift CRUD routes tests"
Cohesion: 0.10
Nodes (35): _lift_from_row must carry bodyweight_pct from the DB row into the Lift     data, test_lift_from_row_maps_bodyweight_pct_and_progression(), create_lift persists load_model/mode (ADR 0005) and round-trips bodyweight_pct., test_create_lift_stores_load_model_mode_and_bodyweight_pct(), _lift(), _t2_lift(), _t2_lift_with_incr(), test_create_lift_via_post() (+27 more)

### Community 12 - "Plan view & tonnage export"
Cohesion: 0.13
Nodes (35): test_recompute_sbs_is_noop(), _fresh(), advance_week must not reset reseeded_cycle to 0 every week (ADR 0002)., Omitting bodyweight_pct must default to 0.0 (legacy callers)., is_legal_combo guard: pure_bodyweight must pair with none, not sbs., Same exercise on different days = two independent rows (keyed by id, not name)., test_append_history_and_list(), test_create_lift_accepts_incr_and_round_trips() (+27 more)

### Community 13 - "Repository tests"
Cohesion: 0.23
Nodes (15): load_profile(), load_state(), profile_from_dict(), profile_to_dict(), YAML load/save for Profile and ProgramState., save_profile(), save_state(), state_from_dict() (+7 more)

### Community 14 - "1RM estimation formulas"
Cohesion: 0.16
Nodes (21): best_1rm(), brzycki(), epley(), est1rm_from_history(), estimate_1rm(), Estimated 1RM = mean of Epley, Brzycki, Wathan (top-3 authoritative formulas)., Mean of the three formulas. Most accurate at reps <= 10., Return (working_weight, reps) of the history entry with the highest     estimat (+13 more)

### Community 15 - "Program advance & week plan"
Cohesion: 0.08
Nodes (28): A lift with this week's last-set logged renders its tonnage inline., Same name on two days must render each day's own weight (id-keyed, not clobbered, Week 1 -> no last week -> tonnage shows 首次., A lift whose last-set is not yet logged shows no tonnage fragment., Filling the last-set returns live est1RM + tonnage in the same fragment., Clearing the last-set returns 200 with empty body so .save-ok is wiped., Two-week t2: past tonnage uses the replayed target; Δ% renders with arrow + colo, 手机离线导出含与桌面同源的 容量 片段（经 _by_day -> _live_html 接出）。     week 1 -> 首次 标记，不除零。fixtur (+20 more)

### Community 16 - "CLI entry points"
Cohesion: 0.12
Nodes (28): build_parser(), cmd_init(), cmd_next(), cmd_show(), cmd_week(), _load(), CLI entry: init / week / next / show., run() (+20 more)

### Community 17 - "Tier switch service tests"
Cohesion: 0.17
Nodes (22): Row, D6：mode 切换不触碰 lifts.incr 列。, Regression: on a legacy DB whose lifts table has NO incr column     (pre-migrate, ADR 0005 legal-combo guard: a barbell lift cannot switch to ``none``     (none i, linear_t2 derive：incr=5 的动作，起始重量 snap 到 eff_incr=5 网格，而非全局 rounding=2.5。, _seed_with_history(), test_apply_mode_switch_keeps_history_and_writes_state(), test_apply_switch_preserves_incr() (+14 more)
...
Cohesion: 0.08
Nodes (39): One-shot: recompute lift_state.est1rm for bodyweight lifts whose stored value p, Recompute est1rm for every lift with bodyweight_pct > 0. Returns count., recompute_bodyweight_est1rm(), test_migrate_from_xlsx_sets_sbs_lift_kind(), test_migrate_from_yaml(), test_migrate_seeds_bodyweight_and_lift_bodyweight_fields(), _write_yaml(), test_load_schedule_returns_dataclasses() (+31 more)
...
Cohesion: 0.25
Nodes (12): ScheduleRow, Single source for reset-to-default settings + the standard SBS RTF 21-week ladde, _rows(), lookup_schedule(), Cyclic 1..21 schedule-row index for an absolute program week., Return the ScheduleRow for (kind, schedule_week(program_week)).      Raises Ke, schedule_week(), test_lift_and_profile_carry_new_fields() (+4 more)
...
### Community 75 - "app.py"
Cohesion: 0.19
Nodes (20): main(), migrate_from_xlsx(), migrate_from_yaml(), One-shot migration: profile.yaml + state.yaml -> SQLite sbs.db., Apply a Profile to an already-open DB: settings + lifts.      Writes the globa, seed(), Behavior guards (ADR 0004): bodyweight lifts must never compute est1RM/tonnage f, ADR 0004 guard: mode.derive_state (preview + apply path) must thread     bodywei (+12 more)
...
Cohesion: 0.25
Nodes (12): _derive(), _has_col(), main(), migrate_modes(), One-shot: rebuild lifts table with load_model/mode, rename lift_state.tier->mode, Rebuild lifts with load_model/mode. Returns rows migrated (0 if already done)., _old_schema_db(), A pathological legacy row with tier='sbs' AND bodyweight_pct>0 must not     KeyE (+4 more)
...
Cohesion: 0.15
Nodes (19): In-memory data model., SetEntry, Working-weight seam: the single translation point from stored added weight to t, added + bodyweight × bodyweight_pct.      bodyweight_pct == 0.0 for an ordinar, working_weight(), Progression-mode registry: single dispatch point for per-mode behaviour.  Each m, Tie engine rules to lift state; manage history + est1rm + week plan.  ADR 0005, Replay an sbs lift's TM from ``lift.max`` over its history (raw, no rounding), (+11 more)
...
Cohesion: 0.31
Nodes (10): Bodyweight lift (t2 Chin-ups, bw=75, pct=1.0): working weight is 75, not 0., _sbs(), test_live_preview_bodyweight_est1rm_uses_working_weight(), test_live_preview_delta_vs_history(), test_live_preview_negative_delta(), test_live_preview_no_history(), test_live_preview_t2_uses_state_weight(), live_preview() (+2 more)
...
Cohesion: 0.38
Nodes (5): test_snapshot_copies_db(), test_snapshot_filename_format(), Snapshot the SQLite db before destructive operations., Copy src_db to dest_dir/sbs-w<week>-<ts>.db.bak. Creates dest_dir. Returns dest, snapshot()
...
## Knowledge Gaps
- **790 isolated node(s):** `crypto`, `http`, `fs`, `path`, `OPCODES` (+785 more)
...
- **Why does `connect()` connect `Lift CRUD routes tests` to `Flask app & cycle index`, `21-week schedule defaults`, `migrate_incr.py`, `Advance service & DB tests`, `app.py`, `plan.py`, `Plan view & tonnage export`, `00_cold_backup_0756400b.md`, `Program advance & week plan`, `Tier switch service tests`, `TM recompute migration tests`, `Schedule migration tests`, `Incr column migration tests`, `Reseed routes tests`, `Recommendations`, `Anti-patterns to avoid`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `Lift` connect `YAML profile/state I/O` to `working_weight`, `Data schema & state recompute`, `app.py`, `plan.py`, `Repository tests`, `CLI entry points`, `Schedule migration tests`, `Cold-backup xlsx importer`, `ScheduleRow`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `get_settings()` connect `Tier switch service tests` to `Flask app & cycle index`, `21-week schedule defaults`, `Advance service & DB tests`, `app.py`, `plan.py`, `Plan view & tonnage export`, `Lift CRUD routes tests`, `Program advance & week plan`, `TM recompute migration tests`, `Schedule migration tests`, `Anti-patterns to avoid`?**
